    ...
    <div class="container">
        <nav>
            <ul id="menu-top-bar-left" class="nav nav-inline pull-left animate-dropdown flip">
                <li class="menu-item animate-dropdown"><a title="Welcome to Worldwide Electronics Store" href="#">به فروشگاه اینترنتی {{ $options['site_name'] }} خوش آمدید</a></li>
            </ul>
        </nav>
    ...
    <div class="container">
        ...
        <ul class="navbar-mini-cart navbar-nav animate-dropdown nav pull-right flip">
            <li class="nav-item dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-mini-cart" style="right: -100px">
                    <li>
                                ...
                                @if($cart_products)
                                    ...
                                    @foreach ( $items as $item )
                                        <li class="mini_cart_item">
                                            <form action="/cart/remove/{{ $item->variation->id }}" method="POST">
                                                <input type="submit" title="حذف از سبد" class="remove" value="×" style="background: none;width: 20px;height: 20px;padding: 0px" />
                                                @method('delete')
                                                @csrf
                                            </form>
                                            <a href="single-product.html">
                                                <img class="attachment-shop_thumbnail size-shop_thumbnail wp-post-image" src="{{ $item->variation->product->photo }}" alt="{{ $item->variation->product->name }}">
                                            ...
                                            @endphp
                                            <span class="quantity">{{ $item->count }} × <span class="amount">{{ $price }}</span></span>
                                        </li>
                                    @endforeach
                                @else
